...
int elabora(pacchetto traccia[], int  n, int x, int* totf) {
	int val;
	val = addattamento(traccia, n, x);
	*totf = byte(traccia, val);
	verifica(traccia, val);
	return val;
}
int  addattamento(pacchetto traccia[], int  n, int x) {
	int i;
	int j = 0;
	for (i = 0; i < n; i++) {
		if (traccia[i].idt < x) {
...
void verifica(pacchetto traccia[], int val) {
	int i;
	for (i = 0; i < val; i++) {
		printf("%d - %d - %d - %d - %f \n", traccia[i].idi, traccia[i].q, traccia[i].idt, traccia[i].idp, traccia[i].v);
		i++;
...
void risultato(int totf, int toto, pacchetto traccia[], int val) {
	int dif;
	dif = toto - totf;
	stampa(totf);
	printf(" la differenza di grandezza in byte tra il file originale e il file modificato:\n difference de: %d\n", dif);
	scrittura(traccia, val);
}
void scrittura(pacchetto traccia[], int val) {
	int i = 0;
	FILE* f;
